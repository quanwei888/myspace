...
class CMemCache extends CCache {
	...
	public $optConnectTimeout = null;
	public $optPollTimeout = null;
	/**
	 *
	...
	public function init() {
		...
		$cache = $this->getMemCache ();
                if($this->useMemcached && null !== $this->optConnectTimeout)
                {
                    $cache->setOption(Memcached::OPT_CONNECT_TIMEOUT, $this->optConnectTimeout);
                }
                if($this->useMemcached && null !== $this->optPollTimeout)
                {
                    $cache->setOption(Memcached::OPT_POLL_TIMEOUT, $this->optPollTimeout);
                }
                if (count ( $servers )) {
	...
	/**
	 *
	 * @return mixed the memcache instance (or memcached if {@link useMemcached}
	 ...
	 * @return string the value stored in cache, false if the value is not in
	 *         the cache or expired.
	 */
